...
const formatAlertValue = (alert, formatMoney) => {
  if (["zero_tax_invoices", "zero_tax_purchases"].includes(alert.type)) return `${alert.value} registros`;
  if (!alert.value) return null;
  return formatMoney(alert.value);
};

export default function TaxAlertPanel({ alerts = [], formatMoney }) {
  return (
    <Card className="space-y-4">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Alertas fiscales</p>
        <h2 className="mt-1 text-lg font-semibold text-slate-950 dark:text-slate-100">Puntos de revision</h2>
      </div>
      {!alerts.length ? (
        <EmptyState title="Sin alertas fiscales" description="No hay eventos relevantes en este periodo." />
      ) : (
        <div className="space-y-3">
          {alerts.map((alert) => {
            const Icon = icons[alert.severity] || Info;
            const value = formatAlertValue(alert, formatMoney);
            return (
              <div key={`${alert.type}-${alert.title}`} className={`rounded-xl border p-4 ${styles[alert.severity] || styles.info}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {value && <p className="mt-2 text-sm font-semibold">{value}</p>}
                  </div>
                </div>
              </div>
            );
