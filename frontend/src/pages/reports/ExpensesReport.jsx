...
export default function ExpensesReport() {
  ...
  const load = async (page = pagination.page) => {
    setLoading(true);
    try {
      const report = await getReport("expenses", { ...filters, page, limit: pagination.limit });
      setData(report);
      setPagination(report.meta || { ...pagination, page });
      setError("");
    } catch (err) { setError(getErrorMessage(err, "No fue posible cargar gastos")); } finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="print-area space-y-6">
      ...
      <DataTable columns={columns} rows={data.expenses || []} pagination={pagination} onPageChange={load} minWidth="980px" emptyTitle="Sin gastos" />
    </div>
  );
}
