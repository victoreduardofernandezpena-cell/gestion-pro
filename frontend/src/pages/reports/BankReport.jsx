...
export default function BankReport() {
  ...
  const load = async () => { setLoading(true); try { setData(await getReport("bank", filters)); setError(""); } catch (err) { setError(getErrorMessage(err, "No fue posible cargar banco")); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="print-area space-y-6">
      ...
      <section className="space-y-3"><h2 className="text-lg font-semibold">Movimientos</h2><DataTable columns={txColumns} rows={data.transactions || []} minWidth="980px" emptyTitle="Sin movimientos bancarios" /></section>
    </div>
  );
}
